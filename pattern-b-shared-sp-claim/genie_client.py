"""
Genie Conversation API client — Pattern B (one shared SP + OAuth custom identity claim).

Per request, the host app mints an OAuth token from the SINGLE shared SP with
custom_claim=<tenant>. Inside the dynamic view orders_secure_b,
current_oauth_custom_identity_claim() reads that claim and isolates the tenant's rows.
Genie's generated SQL has no tenant predicate.
"""
import time
import base64
import yaml
...
class GenieClient:
    ...
    def ask(self, claim, question, poll_seconds=3, max_polls=60):
        """Ask Genie scoped to the given claim. Returns {text, sql, columns, data, status, api}."""
